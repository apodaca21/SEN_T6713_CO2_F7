...
{
if(co2_value<=CO2_LEVEL_NORMAL)
    {
    return CO2_LEVEL_NORMAL;
    }
else if(co2_value<=CO2_WARNING_MAX)
    {
    return CO2_LEVEL_WARNING;
    }
else {return CO2_LEVEL_CRITICAL;}
}
/* USER CODE BEGIN 4 */
	...
	if (huart->Instance == USART2) {
		...
		if( rcv_buff[0] == 0x15 && rcv_buff[1] == 0x04 && rcv_buff[2] == 0x02) {
			...
			if( new_co2 != co2 ) {
				...
				switch(status)
				{
                case CO2_LEVEL_NORMAL:
                    sprintf(message,"CO2: Normal \r\n",co2);break;
                case CO2_LEVEL_WARNING:
                    sprintf(message,"CO2: Peligroso \r\n",co2);break;
                case CO2_LEVEL_CRITICAL:
                    sprintf(message,"CO2: Critico \r\n",co2);break;
				}
